 ...
 */
class NricUploadInteractor extends Interactor<NricUploadContract.Presenter>
        implements NricUploadContract.Interactor {

  NricUploadInteractor(NricUploadContract.Presenter presenter) {
    super(presenter);
  }

  @Override
  public void saveNricProfile(long idNricFront, long idNricBack, String nric, CommonCallback<User> commonCallback) {
    ServiceBuilder.getInstance().getService().saveNricProfile(nric, idNricFront, idNricBack).enqueue(commonCallback);
  }
  ...
  @Override
  public void saveUserId(String idType, String userId, long frontImage, long backImage, CommonCallback<User> commonCallback) {
    ServiceBuilder.getInstance().getService().saveIdUser(userId, idType, frontImage, backImage).enqueue(commonCallback);
  }
}
